...
const add = (type, position) => {
    if (type === "row") {
        if (STATUS.focussedCell) {
            ...
            let csvRow = document.getElementById("csv").insertRow(index);
        ...
        if (STATUS.focussedCell) {
            ...
            for (let row of document.getElementById("csv").rows) {
                let csvCell = row.insertCell(cellIndex);
...
const sanitizeColums = longestRow => {
    ...
    for (let row of rows) {
        while (row.cells.length < longestRow) {
            let csvCell = row.insertCell(row.cells.length);
            let input = document.createElement("input");
            input.type = "text";
            input.value = "";
            csvCell.appendChild(input);
